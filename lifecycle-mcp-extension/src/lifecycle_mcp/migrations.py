...
def apply_requirement_decomposition_migration(db_path: str) -> bool:
    """
    Apply migration to add requirement decomposition fields and views
    ...
    try:
        ...
        if 'decomposition_metadata' not in columns:
            # Add decomposition-specific metadata columns
            cursor.execute("ALTER TABLE requirements ADD COLUMN decomposition_metadata TEXT")
            cursor.execute("ALTER TABLE requirements ADD COLUMN decomposition_source TEXT CHECK (decomposition_source IN ('manual', 'llm_automatic', 'llm_suggested'))")
            cursor.execute("ALTER TABLE requirements ADD COLUMN complexity_score INTEGER CHECK (complexity_score BETWEEN 1 AND 10)")
            cursor.execute("ALTER TABLE requirements ADD COLUMN scope_assessment TEXT CHECK (scope_assessment IN ('single_feature', 'multiple_features', 'complex_workflow', 'epic'))")
            cursor.execute("ALTER TABLE requirements ADD COLUMN decomposition_level INTEGER DEFAULT 0 CHECK (decomposition_level BETWEEN 0 AND 3)")
            ...
            cursor.execute("CREATE INDEX idx_requirement_dependencies_parent ON requirement_dependencies(depends_on_requirement_id, dependency_type)")
            cursor.execute("CREATE INDEX idx_requirements_decomposition ON requirements(decomposition_level, complexity_score, scope_assessment)")
            
            # Create triggers for decomposition validation and level setting
            cursor.execute("""
            ...
            conn.commit()
            print("Requirement decomposition migration applied successfully")
            return True
        else:
            print("Requirement decomposition migration already applied")
            return True
            
    except Exception as e:
        print(f"Error applying requirement decomposition migration: {e}")
        return False
    finally:
        if 'conn' in locals():
            conn.close()
...
def apply_all_migrations(db_path: str) -> bool:
    ...
    migrations = [
        ...
        (3, "Requirement decomposition support", apply_requirement_decomposition_migration)
    ]
