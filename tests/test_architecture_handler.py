...
class TestArchitectureHandler:
    ...
    async def test_create_architecture_decision_success(
        ...
        records = architecture_handler.db.get_records(
            "architecture", "*", "id = ?", ["ADR-0001"]
        )
    ...
    def test_add_architecture_review_not_found(self, architecture_handler):
        ...
        result = architecture_handler._add_architecture_review(
            architecture_id="ADR-9999",
            comment="Test review"
        )
    ...
    async def test_handle_tool_call_routing(
        ...
        result = await architecture_handler.handle_tool_call(
            "create_architecture_decision", sample_architecture_data
        )
        assert len(result) == 1
        assert "SUCCESS" in result[0].text
